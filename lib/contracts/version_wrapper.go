...
func (vw *VersionedWrapper) GetReserveRate(block int64, rsvAddr ethereum.Address, srcs, dest []ethereum.Address) ([]*big.Int, []*big.Int, error) {
	if block == 0 {
		...
		return vw.WrapperContractV2.GetReserveRate(&bind.CallOpts{BlockNumber: big.NewInt(block)}, rsvAddr, srcs, dest)
	}
	//default case: V1 contract.
	return vw.WrapperContractV1.GetReserveRate(&bind.CallOpts{BlockNumber: big.NewInt(block)}, rsvAddr, srcs, dest)
}
